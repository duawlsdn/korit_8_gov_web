...
public class HttpStudyController3 {
    ...
    /*
        GET    : 주소에 값(파라미터)을 입력한다.
            특징 :
                - 파라미터는 주소의 ? 뒤에 따ㅏ오는 key value 값을 의미
                - ex) http://localhost8080/api/~~~?파라미터
                - 파라미터는 여러개의 데이터르 요청시 &로 구분
        ...
        PUT    : JSON 으로 값을 입력한다.
            특징 :
                - JSON 으로 값을 받기 위해서는 @RequestBody 를 추가해야 한다.
                - 키 값으로 받기 때문에 Map<> 사용 가능
    ...
    public String post(@RequestBody Map<String, Object> data) {
        ...
        List<Object> list = (List<Object>) data3;
    ...
    /*
        ...
        post 요청
        - /api/students
        응답 : {status: "성공", data : {Studen객체}}
     */
    @GetMapping("/study/students")
    public List<Student> gS(SearchStudyDto searchStudyDto) {
...
//                new Student("이수원",25));
        ...
        if (searchStudyDto.getSearchValue() != null && !searchStudyDto.getSearchValue().isBlank()) {
            foundStudents = students.stream().filter(s -> s.getName().contains(searchStudyDto.getSearchValue())).toList();
        }
    ...
    public Map<String, Object> poS(@RequestBody Student s) {
    ...
    // /api/study/delete/10 => 10 번의 id 값 을 가진 데이터 삭제
